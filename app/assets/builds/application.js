...
%o`, message, error2, detail);
  ...
  var optionSelector2 = "[role='option']:not([aria-disabled])";
  var activeSelector2 = "[aria-selected='true']";
  var Autocomplete2 = class _Autocomplete extends Controller {
    static targets = ["input", "hidden", "results"];
    static classes = ["selected"];
    static values = {
      ready: Boolean,
    ...
    onResultsClick = (event) => {
      if (!(event.target instanceof Element))
        return;
      const selected = event.target.closest(optionSelector2);
      if (selected)
        this.commit(selected);
    };
    onResultsMouseDown = () => {
    ...
    identifyOptions() {
      const prefix = this.resultsTarget.id || "stimulus-autocomplete";
      const optionsWithoutId = this.resultsTarget.querySelectorAll(`${optionSelector2}:not([id])`);
      optionsWithoutId.forEach((el) => el.id = `${prefix}-option-${_Autocomplete.uniqOptionId++}`);
    }
    hideAndRemoveOptions() {
      this.close();
    ...
    get options() {
      return Array.from(this.resultsTarget.querySelectorAll(optionSelector2));
    }
    get selectedOption() {
      return this.resultsTarget.querySelector(activeSelector2);
    }
    get selectedClassesOrDefault() {
      return this.hasSelectedClass ? this.selectedClasses : ["active"];
    }
  ...
  application.register("autocomplete", Autocomplete2);
